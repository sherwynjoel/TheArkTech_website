...
/**
 * Cards Shader Effect (21st.dev), with optional additions for this site, all
 * defaulting to the original behaviour:
 *   front / back  – custom face content in place of the demo bank-card art
 *   design        – a fixed design index instead of a random one per click
 *   tapToFlip     – a click/tap toggles the flip (phones have no hover); hover
 *                   is then only honoured on devices that really have one
 *   backShader    – false skips the blurred shader on the back (one less
 *                   WebGL canvas; the back keeps its dark base colour)
 *   cardWidth     – a fixed card width in px (height follows the card ratio)
 *                   instead of the window-based size
 *   ariaLabel     – accessible name for the card control
 *   className     – extra classes for the root (e.g. bg-transparent), merged with cn()
 */
...
const CARD_RATIO = 1.5925; // Standard credit card ratio
...
export function CardsShader({
  ...
  // Site addition: a tap/click can hold the card flipped where there is no hover.
  const isFlippedRef = useRef<boolean>(false);
  const canHoverRef = useRef<boolean>(true);
  const [designIndex, setDesignIndex] = useState<number>(design ?? 0);
  ...
  const [metrics, setMetrics] = useState({
    cardW: cardWidth ?? 336,
    cardH: cardWidth ? Math.round(cardWidth / CARD_RATIO) : 211, // 1.59 standard credit card ratio
  });
  ...
  useEffect(() => {
    if (cardWidth) {
      setMetrics({ cardW: cardWidth, cardH: Math.round(cardWidth / CARD_RATIO) });
      return;
    }
    const handleResize = () => {
  ...
  const renderLoop = () => {
    ...
    const targetFlip = isHoveredRef.current || isFlippedRef.current ? 180 : 0;
  ...
  return (
    <div className={cn("absolute inset-0 bg-[#000000] text-white flex items-center justify-center overflow-hidden select-none", className)}>
      ...
      >
        ...
        <div
          ...
          onKeyDown={(e) => {
            if (tapToFlip && (e.key === "Enter" || e.key === " ")) {
              e.preventDefault();
              isFlippedRef.current = !isFlippedRef.current;
            }
          }}
          onMouseEnter={() => {
            if (tapToFlip && !canHoverRef.current) return;
